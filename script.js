...
function validateAllInputs() {
    ...
    if (!selectedGender) {
        displayInputError(genderFemale, genderError, 'Please select your gender.');
        isValid = false;
    } else {
    ...
    if (isNaN(weight) || weight <= 0) {
        displayInputError(weightInput, weightError, 'Please enter a valid weight (e.g., > 0 kg).');
        isValid = false;
    } else {
    ...
    if (isNaN(height) || height <= 0) {
        displayInputError(heightInput, heightError, 'Please enter a valid height (e.g., > 0 cm).');
        isValid = false;
    } else {
    ...
    if (isNaN(age) || age <= 0 || age > 120) {
        displayInputError(ageInput, ageError, 'Please enter a valid age (1-120 years).');
        isValid = false;
    } else {
    ...
    if (!selectedActivity) {
        displayInputError(activityRadios[0], activityError, 'Please select your activity level.');
        isValid = false;
    } else {
    ...
    if (!selectedGoal) {
        displayInputError(goalRadios[0], goalError, 'Please select your goal.');
        isValid = false;
    } else {
...
/**
 * Handles the calculation of BMR, TDEE, and Final Calorie Need when the single "Calculate" button is clicked.
 */
calculateAllBtn.addEventListener('click', () => {
    hideMessageBox(); // Hide any previous global messages

    if (!validateAllInputs()) {
        showMessageBox('Please correct the highlighted input errors.', 'error');
